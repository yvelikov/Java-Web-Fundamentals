...
abstract class BaseDynamicHandler extends BaseHandler {
    private static final String VIEW_PATH = System.getProperty("user.dir") + "\\src\\org\\softuni\\main\\casebook\\resources\\templates\\";
    private static final String BASE_VIEW_FULL_PATH = System.getProperty("user.dir") + "\\src\\org\\softuni\\main\\casebook\\resources\\base-template.html";
    private static final String HEADER_VIEW_PATH = System.getProperty("user.dir") + "\\src\\org\\softuni\\main\\casebook\\resources\\headers\\";
    ...
    private String getView(String viewName) {
        try {
            List<String> content = Files.readAllLines(Paths.get(VIEW_PATH + viewName + VIEW_EXTENSION));
            return String.join("", content);
        } catch (IOException e) {
            return null;
        }
    ...
    private String getHeaderView(String view) {
        try {
            List<String> content = Files.readAllLines(Paths.get(HEADER_VIEW_PATH + view + VIEW_EXTENSION));
            return String.join("", content);
        } catch (IOException e) {
            return null;
        }
